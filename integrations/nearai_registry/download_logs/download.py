...
def convert_nearai_logs_to_canonical(entry_info: EntryInformation, from_path: Path, to_path: Path):
...
def main():
    parser = argparse.ArgumentParser(description="Download and convert nearai registry logs")
    parser.add_argument(
        "--namespaces",
        type=str,
        default="",
    ...
    parser.add_argument("--limit", type=int, default=200, help="Maximum number of entries to download")
    ...
    namespaces_list = [ns.strip() for ns in namespaces_str.split(",") if ns.strip()] if namespaces_str else []
    ...
    if len(entries) >= limit:
        print(f"There are more than {limit} logs entries. Downloading {limit} most recent ones..")
    ...
    logs_dir = Path("~/.nearai/logs").expanduser()
    ...
    for entry in entries:
        ...
        try:
            ...
            convert_nearai_logs_to_canonical(entry_info=entry, from_path=nearai_logs, to_path=canonical_logs)
